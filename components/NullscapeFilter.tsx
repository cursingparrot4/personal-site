...
type Props = {
  ...
  atmosphere?: number;
  /** Ordered dither + diagonal colour bands. */
  dither?: boolean;
  /** Animated film grain. */
  grain?: boolean;
 ...
 *   texture    (z 30) above content: scanlines, dither, bands, grain, colour grade
 *
 * Mounted once in app/layout.tsx. Page content sits at z-index 1 (see globals.css)
 * so it renders between the two stacks. Purely decorative: aria-hidden and
 * pointer-events: none throughout.
 */
export function NullscapeFilter({
  atmosphere = 0.2,
  dither = true,
  grain = true,
  beamRgb = "52 197 221",
  ...
  return (
    <>
      ...
      <div aria-hidden="true" className={styles.texture} style={vars}>
        ...
        {dither ? <div className={styles.dither} /> : null}
        {dither ? <div className={styles.bands} /> : null}
        {grain ? <div className={styles.grain} /> : null}
